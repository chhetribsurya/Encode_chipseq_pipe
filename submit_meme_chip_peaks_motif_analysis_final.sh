...
for i in "${!LIB_REP1[@]}"; do
	...
	bsub $BSUB_OPTIONS -o "$RUN_PATH/test_motif_corrected_idr.${R1}_${R2}.truerep.out" -J "Motif analysis from IDR of $R1 $R2 " $RUN_PATH/meme_chip_peaks_motif_analysis_final.sh $IDR_DIR/${R1}.filt.nodup.srt.SE_VS_${R2}.filt.nodup.srt.SE.IDR0.02.filt.narrowPeak.gz $R1 $R2
